...
def Invers():
    ...
    try:
        ...
        titleC = Label(f4, font=("arial", "9", "bold"), text="Invers(A)", bg="#474E64", fg="#FFFFFF")
        ...
        for i in range(len(A_inverse)):
            for j in range(len(A_inverse[0])):
                entry = Entry(f4, width=5, justify="center")
                entry.grid(row=i + 1, column=j)
                entry.insert(0, round(A_inverse[i, j], 2))  # Bulatkan ke dua desimal
    except:
        # Menampilkan messagebox bila error (matriks tidak memiliki invers)
        messagebox.showinfo(title="informasi", message="Matriks tidak memiliki invers")
